...
const startServer = (port) => {
  app.listen(port,'0.0.0.0', () => {
    ...
    if (err.code === 'EACCES') {
      console.error(`Porta ${port} não tem permissão. Tentando usar a porta 3003...`);
      startServer(3003); // Tentar a porta 3001
    } else if (err.code === 'EADDRINUSE') {
      console.error(`Porta ${port} está em uso. Tentando usar a porta 3003...`);
      startServer(3003); // Tentar a porta 3001
    } else {
      console.error(err);
    }
